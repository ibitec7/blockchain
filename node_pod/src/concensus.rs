...
pub trait Pbft {
    async fn preprepare_phase(&mut self, _pool: Vec<Transaction>,_producer: &BaseProducer){}

    async fn prepare_phase(&mut self, _pkey_store: &HashMap<String, PublicKey>, _primary_msg: Vec<String>,_producer: &BaseProducer){}

    fn commit_phase(&mut self, _pkey_store: &HashMap<String, PublicKey>, _prepare_msg: Vec<String>,_producer: &BaseProducer) -> impl std::future::Future<Output = bool> + Send {async {false} }
}
...
impl Stake {
    pub fn new(node: &Node, stake: f64) -> Self {
...
impl PoS for Node {
    async fn propose_stake(&mut self, producer: &BaseProducer){
        ...
        let record_json = stake.serialize();
